...
def default_guard_define_name_generator(filename, directory, prefix, dirnames):
    altered_filename = filename.upper()
    altered_filename = altered_filename.replace(".", "_")
    
    dirnames_str = "" if dirnames == 0 else "_"
    dirnames_current = directory
    while dirnames_current and dirnames > 0:
        ...
        dirnames_str = "%s%s%s" % ("" if dirnames <= 1 else "_", altered_dirname, dirnames_str)
        dirnames_current = os.path.dirname(dirnames_current)
        dirnames -= 1
    
    return "%s_%s%s" % (prefix, dirnames_str, altered_filename)
...
def main():
    ...
    parser.add_argument("--project", type = str, required = True, help = "Project name used as a prefix for header guards.")
    parser.add_argument("--dirnames", type = int, default = 0, help = "How many directory names should be included in the header guard, default = 0.")
    ...
    for file in files:
        clean_include_guard(file, guard_define_name_generator = lambda filename, directory: default_guard_define_name_generator(filename, directory, args.project, args.dirnames), check_only = args.check)
